...
class Extractor:
    ...
    @staticmethod
    def download_url(html):
        flag, video_list, audio_list = Extractor.urls(html)
        if not flag:
            return False, None, None

        video_list.sort(key=lambda video: int(re.findall(r'^(\d+)p', video["definition"])[0]), reverse=False)
