                           ...
                           AVCodecContext **input_codec_context)
{
    ...
    if (!(input_codec = avcodec_find_decoder((*input_format_context)->streams[audio_stream_index]->codecpar->codec_id))) {
    ...
    error = avcodec_parameters_to_context(avctx, (*input_format_context)->streams[audio_stream_index]->codecpar);
    if (error < 0) {
        avformat_close_input(input_format_context);
                            ...
                            AVCodecContext **output_codec_context)
{
   ...
   if (!(output_codec = avcodec_find_encoder((*output_format_context)->oformat->audio_codec))) {
        fprintf(stderr, "Could not find an AAC encoder %s.\n",(*output_format_context)->streams[0]->codecpar->codec_tag);
        goto cleanup;
    }
    fprintf(stderr, "encoder %s.\n",output_codec->long_name);
                              ...
                              int *data_present, int *finished)
{
    ...
    //in_stream  = input_format_context->streams[input_packet.stream_index];
                                  ...
                                  int frame_size)
{
    ...
    uint8_t ** kjkj=static_cast<uint8_t **>(calloc(output_codec_context->channels,sizeof(**converted_input_samples)));

    if (!(*converted_input_samples =kjkj)) {
...
int ConverToMp3(char * input_file_path, char * out_file_path)
{
    ...
    if (open_output_file(out_file_path, input_codec_context,
